...
int mini_svm_construct_debug_mm_one_page(struct mini_svm_mm *mm) {
	...
	pml4->pdp.pd.memory_2mb_va[0] = (void *)__get_free_pages(GFP_KERNEL | ___GFP_ZERO, get_order(2 * 1024 * 1024));
	...
	pml4->pdp.pd.va[0] = mini_svm_create_entry(pml4->pdp.pd.memory_2mb_pa[0], MINI_SVM_PRESENT_MASK | MINI_SVM_WRITEABLE_MASK | MINI_SVM_USER_MASK | MINI_SVM_LEAF_MASK);
...
fail:
	...
	if (pml4->pdp.pd.memory_2mb_va[0]) {
		free_pages((unsigned long)pml4->pdp.pd.memory_2mb_va[0], get_order(2 * 1024 * 1024));
	}
	return r;
}
