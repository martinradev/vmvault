...
void sevault_mini_run_tests(struct sevault_mini_context *ctx) {
...

	{
		...
		for (i = 0; i < sizeof(all_key_sizes) / sizeof(all_key_sizes[0]); ++i) {
			...
			uint16_t contextId;
			ret = registerContext(gva_to_gpa(&key_page[0]), keylen, NULL, 0, &contextId);
	...
	{
		uint16_t contextId;
		ret = registerContext(gva_to_gpa(&key_page[0]), 100, NULL, 0, &contextId);
		BUG_ON(ret == SevaultMiniReturnResult_Ok);
		ret = registerContext(gva_to_gpa(&key_page[0]), (uint16_t)-1, NULL, 0, &contextId);
		BUG_ON(ret == SevaultMiniReturnResult_Ok);
		ret = registerContext(gva_to_gpa(&key_page[0]), 0, NULL, 0, &contextId);
		BUG_ON(ret == SevaultMiniReturnResult_Ok);
	}
...

	{
		...
		uint16_t contextId;
		for (i = 0; true; ++i) {
			ret = registerContext(gva_to_gpa(&key_page[0]), 16, NULL, 0, &contextId);
...

	{
		...
		// Small block
		{
			...
			ret = encryptDataSingleSgEntry(keyId, SevaultMiniCipher_AesEcb, gva_to_gpa(data_page), 16, gva_to_gpa(output_page));
			BUG_ON(!(ret == SevaultMiniReturnResult_Ok));
			uint8_t expected[] =
			...
			uint16_t keyDecId;
			ret = registerContext(gva_to_gpa(key_page), 16, NULL, 0, &keyDecId);
			BUG_ON(!(ret == SevaultMiniReturnResult_Ok));
			BUG_ON(!(keyDecId >= 0));

			ret = decryptDataSingleSgEntry(keyDecId, SevaultMiniCipher_AesEcb, gva_to_gpa(data_page), 16, gva_to_gpa(&output_page[16]));
			BUG_ON(!(ret == SevaultMiniReturnResult_Ok));
		...
		// Multiple blocks
		{
			...
			BUG_ON(!(ret == SevaultMiniReturnResult_Ok));
			BUG_ON(!(keyId >= 0));
			ret = encryptDataSingleSgEntry(keyId, SevaultMiniCipher_AesEcb, gva_to_gpa(data_page), 96, gva_to_gpa(output_page));
			...
			BUG_ON(!(ret == SevaultMiniReturnResult_Ok));
			BUG_ON(!(keyId >= 0));
			ret = decryptDataSingleSgEntry(keyId, SevaultMiniCipher_AesEcb, gva_to_gpa(output_page), 96, gva_to_gpa(&output_page[96]));
		...
		// Invalid block size
		{
			...
			BUG_ON(!(ret == SevaultMiniReturnResult_Ok));
			BUG_ON(!(keyId >= 0));
			ret = encryptDataSingleSgEntry(keyId, SevaultMiniCipher_AesEcb, gva_to_gpa(data_page), 44, gva_to_gpa(output_page));
			BUG_ON(!(ret != SevaultMiniReturnResult_Ok));

			ret = decryptDataSingleSgEntry(keyId, SevaultMiniCipher_AesEcb, gva_to_gpa(data_page), 44, gva_to_gpa(output_page));
			BUG_ON(!(ret != SevaultMiniReturnResult_Ok));
		}
		...
		// Single block
		{
			...
			ret = encryptDataSingleSgEntry(keyId, SevaultMiniCipher_AesCbc, gva_to_gpa(data_page), 16, gva_to_gpa(output_page));
			BUG_ON(!(ret == SevaultMiniReturnResult_Ok));
			BUG_ON(!(memcmp(output_page, expected, 16) == 0));

			ret = registerContext(gva_to_gpa(key_page), 16, gva_to_gpa(iv_page), 16, &keyId);
			BUG_ON(!(ret == SevaultMiniReturnResult_Ok));
			ret = decryptDataSingleSgEntry(keyId, SevaultMiniCipher_AesCbc, gva_to_gpa(output_page), 16, gva_to_gpa(&output_page[16]));
			BUG_ON(!(ret == SevaultMiniReturnResult_Ok));
		...
		// Multiple blocks
		{
			...
			ret = encryptDataSingleSgEntry(keyId, SevaultMiniCipher_AesCbc, gva_to_gpa(data_page), 96, gva_to_gpa(output_page));
			BUG_ON(!(ret == SevaultMiniReturnResult_Ok));
			BUG_ON(!(memcmp(output_page, expected, 96) == 0));
			...
			BUG_ON(!(ret == SevaultMiniReturnResult_Ok));
			BUG_ON(!(keyId >= 0));
			ret = decryptDataSingleSgEntry(keyId, SevaultMiniCipher_AesCbc, gva_to_gpa(output_page), 96, gva_to_gpa(&output_page[96]));
			BUG_ON(!(ret == SevaultMiniReturnResult_Ok));
