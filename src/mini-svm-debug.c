...
void mini_svm_run_tests(struct mini_svm_context *ctx) {
...

	{
		...
		for (i = 0; i < sizeof(all_key_sizes) / sizeof(all_key_sizes[0]); ++i) {
			...
			uint16_t contextId;
			ret = registerContext(&key_page[0], keylen, NULL, 0, &contextId);
	...
	{
		uint16_t contextId;
		ret = registerContext(&key_page[0], 100, NULL, 0, &contextId);
		BUG_ON(ret == MiniSvmReturnResult_Ok);
		ret = registerContext(&key_page[0], (uint16_t)-1, NULL, 0, &contextId);
		BUG_ON(ret == MiniSvmReturnResult_Ok);
		ret = registerContext(&key_page[0], 0, NULL, 0, &contextId);
		BUG_ON(ret == MiniSvmReturnResult_Ok);
	}
	...
	// Check iv
	{
		...
		ret = registerContext(&key_page[0], 16, &iv_page[0], 32, &contextId);
		BUG_ON(ret == MiniSvmReturnResult_Ok);

		ret = registerContext(&key_page[0], 16, &iv_page[0], 16, &contextId);
		BUG_ON(ret != MiniSvmReturnResult_Ok);
		ret = removeContext(contextId);
		BUG_ON(ret != MiniSvmReturnResult_Ok);

		ret = registerContext(&key_page[0], 16, &iv_page[0], 4U, &contextId);
		BUG_ON(ret == MiniSvmReturnResult_Ok);
	}

	{
		...
		uint16_t contextId;
		for (i = 0; true; ++i) {
			ret = registerContext(&key_page[0], 16, NULL, 0, &contextId);
...

	{
		...
		// Small block
		{
			...
			uint16_t keyId;
			ret = registerContext(key_page, 16, NULL, 0, &keyId);
			BUG_ON(!(ret == MiniSvmReturnResult_Ok));
			BUG_ON(!(keyId >= 0));
			...
			ret = encryptData(keyId, MiniSvmCipher_AesEcb, data_page, 16, output_page);
			BUG_ON(!(ret == MiniSvmReturnResult_Ok));
			uint8_t expected[] =
			...
			uint16_t keyDecId;
			ret = registerContext(key_page, 16, NULL, 0, &keyDecId);
			BUG_ON(!(ret == MiniSvmReturnResult_Ok));
			BUG_ON(!(keyDecId >= 0));

			ret = decryptData(keyDecId, MiniSvmCipher_AesEcb, data_page, 16, &output_page[16]);
			BUG_ON(!(ret == MiniSvmReturnResult_Ok));
		...
		// Multiple blocks
		{
			...
			BUG_ON(!(ret == MiniSvmReturnResult_Ok));
			BUG_ON(!(keyId >= 0));
			ret = encryptData(keyId, MiniSvmCipher_AesEcb, data_page, 96, output_page);
			...
			BUG_ON(!(ret == MiniSvmReturnResult_Ok));
			BUG_ON(!(keyId >= 0));
			ret = decryptData(keyId, MiniSvmCipher_AesEcb, output_page, 96, &output_page[96]);
		...
		// Invalid block size
		{
			...
			BUG_ON(!(ret == MiniSvmReturnResult_Ok));
			BUG_ON(!(keyId >= 0));
			ret = encryptData(keyId, MiniSvmCipher_AesEcb, data_page, 44, output_page);
			BUG_ON(!(ret != MiniSvmReturnResult_Ok));

			ret = decryptData(keyId, MiniSvmCipher_AesEcb, data_page, 44, output_page);
			BUG_ON(!(ret != MiniSvmReturnResult_Ok));
		}
		...
		// Single block
		{
			...
			uint16_t keyId;
			ret = registerContext(key_page, 16, iv_page, 16, &keyId);
			BUG_ON(!(ret == MiniSvmReturnResult_Ok));
			BUG_ON(!(keyId >= 0));
			const uint8_t expected[] = { 0xaaU, 0x1aU, 0x18U, 0xffU, 0x55U, 0x61U, 0x5fU, 0x61U, 0x22U, 0xf2U, 0x87U, 0x48U, 0x65U, 0xc8U, 0x1bU, 0xfcU };
			ret = encryptData(keyId, MiniSvmCipher_AesCbc, data_page, 16, output_page);
			BUG_ON(!(ret == MiniSvmReturnResult_Ok));
			BUG_ON(!(memcmp(output_page, expected, 16) == 0));

			ret = registerContext(key_page, 16, iv_page, 16, &keyId);
			BUG_ON(!(ret == MiniSvmReturnResult_Ok));
			ret = decryptData(keyId, MiniSvmCipher_AesCbc, output_page, 16, &output_page[16]);
			BUG_ON(!(ret == MiniSvmReturnResult_Ok));
		...
		// Multiple blocks
		{
			...
			ret = encryptData(keyId, MiniSvmCipher_AesCbc, data_page, 96, output_page);
			BUG_ON(!(ret == MiniSvmReturnResult_Ok));
			BUG_ON(!(memcmp(output_page, expected, 96) == 0));
			...
			BUG_ON(!(ret == MiniSvmReturnResult_Ok));
			BUG_ON(!(keyId >= 0));
			ret = decryptData(keyId, MiniSvmCipher_AesCbc, output_page, 96, &output_page[96]);
			BUG_ON(!(ret == MiniSvmReturnResult_Ok));
